...
function isNegative(text) {
  ...
  return containsAny(t, [
    ...
    // "something else" is not a refusal. The final wrap-up prompt even asks
    // "Is there something else…", so treating that phrase as a hard no hung up
    // and dropped added details / wiped confirmed addresses.
    "standard service", "normal service", "regular service", "another time", "different time",
...
if (process.env.BLUE_CALLER_TEST_SOMETHING_ELSE === "1") {
  const casesPath = path.join(__dirname, "something_else_intent_cases.json");
  let cases;
  try {
    cases = JSON.parse(fs.readFileSync(casesPath, "utf8"));
  } catch (err) {
    console.error("Could not load something_else_intent_cases.json:", err.message);
    process.exit(1);
  }
  ...
  function runSomethingElseIntentTests() {
    let passed = 0;
    let total = 0;
    const failures = [];
    ...
    return Promise.resolve().then(async () => {
      for (const tc of cases) {
        ...
        if (kind === "wrap_up") {
        ...
        } else if (kind === "final_question") {
          const sessionKey = `something-else-final-${total}`;
          ...
          const joined = Array.isArray(caller.additionalIssues) ? caller.additionalIssues.join(" | ") : "";
          if (tc.expect_additional_issue_includes && !joined.toLowerCase().includes(String(tc.expect_additional_issue_includes).toLowerCase())) {
            caseFailures.push(`expected additional issue to include ${JSON.stringify(tc.expect_additional_issue_includes)} but got ${JSON.stringify(joined)}`);
          }
        } else if (kind === "confirm_address") {
          const sessionKey = `something-else-address-${total}`;
          const ws = makeTestWs(sessionKey);
          const caller = getOrCreateCaller(sessionKey);
          caller.lastStep = "confirm_address";
          caller.leadType = "service";
          ...
          const sessionKey = `something-else-leak-${total}`;
          const ws = makeTestWs(sessionKey);
          const caller = getOrCreateCaller(sessionKey);
          caller.lastStep = "leak_emergency_choice";
          caller.leadType = "service";
      ...
      console.log(`\nPassed ${passed} of ${total} something-else intent cases.`);
      process.exit(failures.length ? 1 : 0);
    }).catch((err) => {
      console.error("something-else intent tests failed:", err);
      process.exit(1);
    });
  }

  runSomethingElseIntentTests();
...
if (!process.env.BLUE_CALLER_TEST_WRAP_UP && !process.env.BLUE_CALLER_TEST_SOMETHING_ELSE) {
