...
/** "new" as an extra problem, or a recently bought unit that already failed — not a new-install quote. */
function isNewProblemOrFailedNewUnit(text) {
  const t = normalizeIntentText(text || "");
  if (!t) return false;
  if (/\bnew\s+(problem|issues?|trouble|symptom|symptoms)\b/.test(t)) return true;
  if (!/\b(?:my|our|the|this|that)\s+(?:brand\s+)?new\b/.test(t)) return false;
  return containsAny(t, [
    "failed", "failing", "fails", "failure",
    "acting up", "acts up",
    "error code", "giving me trouble", "giving us trouble",
    "having issues", "having a problem", "having problems",
    "quit working", "went out", "went bad", "died", "is dead"
  ]);
}

/** Word-boundary appliance/fixture names so "range" does not match inside "strange"/"orange"/"arrangement". */
function hasQuotedApplianceOrFixtureWord(text) {
  const t = normalizeIntentText(text || "");
  if (!t) return false;
  return /\b(?:appliances?|refrigerators?|fridges?|dishwashers?|stoves?|ovens?|ranges?|cooktops?|washers?|dryers?|microwaves?|garbage\s+disposals?|water\s+heaters?|toilets?|faucets?)\b/.test(t);
}
...
function isQuoteIntent(text) {
  ...
  // "a new problem with my washer" / "my new fridge failed" are repairs, not project quotes.
  if (isNewProblemOrFailedNewUnit(text)) return false;
  if (containsAny(t, ["install", "installation", "replace", "replacement", "new"]) && hasQuotedApplianceOrFixtureWord(t)) return true;
  return false;
}
...
if (process.env.BLUE_CALLER_TEST_QUOTE_NEW_PROBLEM === "1") {
  const casesPath = path.join(__dirname, "quote_new_problem_cases.json");
  let cases;
  try {
    cases = JSON.parse(fs.readFileSync(casesPath, "utf8"));
  } catch (err) {
    console.error("Could not load quote_new_problem_cases.json:", err.message);
    process.exit(1);
  }
  ...
  console.log(`\nPassed ${passed} of ${cases.length} quote-new-problem cases.`);
